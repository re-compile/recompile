...
run_native_case() {
    ...
    if ! "$runner_path" run --native "build/user-samples/${name}" --output "$output_dir" >"$log_path" 2>&1; then
...
assert_summary() {
    ...
    python3 - "$summary_path" "$expected_total" "$expected_class" "$expected_escalation_class" <<'PY'
import json
import pathlib
import sys
...
printf '[summarize] finding crashpack\n'
run_native_case copy_overrun_case
assert_summary "${output_root}/copy_overrun_case" 1 heap_overflow __none__
...
printf '[summarize] escalated crashpack\n'
